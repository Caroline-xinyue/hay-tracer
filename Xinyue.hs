...
shader :: Ray -> [Object] -> Color
shader _ []     = Vec3 127.5 127.5 127.5
shader ray@(Ray origin direction) objs = case checkIntersect ray objs of
     Nothing                 -> Vec3 127.5 127.5 127.5
     Just (min_obj, min_pos) -> phong ray point min_obj where
...
trace :: Ray -> [Surface] -> [Object] -> Int -> Color
{-
trace ray@(Ray origin direction) depth =
  if depth > 20 then Color 127.5 127.5 127.5
  else do
    intersect <- checkIntersect ray
-}
...
sendRay :: Image -> Vec3 -> Camera -> M.Matrix Vec3
